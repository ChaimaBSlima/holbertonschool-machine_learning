...
def determinant(matrix):
    ...
    param matrix:
    A list of lists representing the square matrix whose determinant is to be calculated. 
    It must be a square matrix (n x n), where each inner list represents a row.
...
The determinant of the matrix as a number (int or float).
	...
	return sum(matrix[0][i] * (-1) ** i * determinant([row[:i] + row[i+1:] for row in matrix[1:]]) for i in range(len(matrix)))
